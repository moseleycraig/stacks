...
(define-private (validate-bitcoin-block (block-hash (buff 32)) (btc-block-height uint))
  (and
    (> (len block-hash) u0)                    ;; Bitcoin block hash exists
    (> btc-block-height u0)                    ;; Bitcoin block height is valid
    (> stacks-block-height u0)                 ;; Stacks block height is valid
    ;; Optional: Check that Bitcoin and Stacks heights are reasonable
    ;; (< btc-block-height (+ stacks-block-height u1000000)) ;; Some reasonable upper bound
    true
  )
)
...
(define-private (is-score-stale (last-update uint))
    (> (- block-height last-update) SCORE_STALENESS_THRESHOLD)
)
