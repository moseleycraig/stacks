...
;; The start function will be called by the contract owner to initialise the vault.
;; It is a simple function that updates the two variables with the proper guards in place.

(define-public (start (new-members (list 100 principal)) (new-votes-required uint))
	(begin
		(asserts! (is-eq tx-sender contract-owner) err-owner-only)
		(asserts! (is-eq (len (var-get members)) u0) err-already-locked) ;; prevent XS members
...
;; The vote function is even more straightforward. 
;; All we have to do is make sure the tx-sender is one of the members.
;; We can do that by checking if the tx-sender is present in the members list by using the built-in index-of function.
;; It returns an optional type, so we can simply check if it returns a (some ...), rather than a none.
...
;; While we are at it, let us also add a read-only function to retrieve a vote.
...
;; Since we want to count the number of positive votes, we should increment the accumulator value only when the vote for the principal is true.
;; There is no built-in function that can do that so we have to create a custom accumulator as a private function.

(define-private (tally (member principal) (accumulator uint))
	(if (get-vote member tx-sender) (+ accumulator u1) accumulator) 
)

;; (if (get-vote member tx-sender) --- The condition (boolean expression) ^
